...
if (!empty($row['photo_path']) && file_exists($row['photo_path'])) {
    $pdf->Image($row['photo_path'], 165, 35, 28, 34);
...
$pdf->Cell(0, 6, 'PERSONAL INFORMATION', 1, 1);
...
$pdf->MultiCell(
    0,
    6,
    "This is to certify that the following documents have been received from {$row['student_name']} for admission to BE in the Branch {$row['allotted_branch']} for the academic year {$academicYear}.",
    0
);
...
// ===============================
$qrData = $row['application_id'].' | '.$row['mobile'].' | '.$row['allotted_branch'].' | '.$row['admission_through'];
